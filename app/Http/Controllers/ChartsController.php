...
use App\CometView;
use Carbon\Carbon;
use App\Classes\FiscalCalculator;
...
class ChartsController extends Controller
{
  ...
  {
    $earliestDataDate = min([
      Cache::get('earliestCometViewDate'),
      Cache::get('earliestCometCompletionDate')
    ]);

    $earliestFiscalYear = (new FiscalCalculator(4, 1))->get($earliestDataDate)->year;

    $latestFiscalYear = (new FiscalCalculator(4, 1))->get()->year;

    if ($earliestFiscalYear === $latestFiscalYear) {
      return [
        $earliestFiscalYear
      ];
    }
    ...
    for ($i = $earliestFiscalYear; $i <= $latestFiscalYear; $i++) {
      $fiscalYears[] = $i;
    }
    $quarters = $this->get_quarters('2010-04-05', '2015-05-01');
       
    dd($quarters, $fiscalYears);
  }

  // get month name from number
  protected function month_name($month_number){
    return date('F', mktime(0, 0, 0, $month_number, 10));
  }

  // get get last date of given month (of year)
  protected function month_end_date($year, $month_number){
    return date("t", strtotime("$year-$month_number-1"));
  }

  // return two digit month or day, e.g. 04 - April
  protected function zero_pad($number){
    if($number < 10)
      return "0$number";
  ...
  function get_quarters($start_date, $end_date){
    ...
    for( $y = $start_year; $y <= $end_year; $y++ ){
      ...
      for($q=$quarter; $q<=$max_qtr; $q++){
        ...
        $endDateObj = new Carbon($y . '-' . $end_month_num . '-' . $this->month_end_date($y, $end_month_num));

        $current_quarter->period = "Quarter ending {$endDateObj->isoFormat('MMMM Do YYYY')}";
